...
function convertExt(fpath) {
  return fpath.replace(/src\//, 'dist/').replace(/(.css|.less)$/, '.wxss')
}

function mkdirsSync(path) {
  let stash = typeof path === 'string' ? path.split('/') : path;
  let pathStr = ''
  stash.forEach((ele, index) => {
    {
      pathStr += ele;
...
function writeFile(fpath) {
  ...
  less.render(content, {
    ...
    let path = convertExt(fpath);
    let filename;
    if (!fs.existsSync(path)) {
      let stash= path.split('/');
      stash.pop()
      mkdirsSync(stash);
    }
...
watch.on('all', (evt, fpath) => {
  ...
  if (split.length === 2) {
    ...
    switch (ext) {
      ...
      default: {
        let dist=fpath.replace('src/', 'dist/')
        if (!fs.existsSync(dist)) {
          let stash = dist.split('/');
          stash.pop()
          mkdirsSync(stash)
        }
        fs.writeFileSync(dist, fs.readFileSync(fpath))
        break;
      }
    }
  }
